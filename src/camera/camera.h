...
class Camera {
public:
  Camera(double fov, const Point3 &from, const Point3 &at, const Vec3 &_vup)
      : vfov(fov), lookfrom(from), lookat(at), vup(_vup) {}
  Camera() = default;
...
public:
  ...
  double focus_dist =
      10; // Distance from Render lookfrom point to plane of perfect focus
...
public:
  CameraList() = default;
  CameraList(std::shared_ptr<Camera> &camera) { add_camera(camera); }
  ...
  void delete_camera(size_t id) {
    ...
    auto it =
        std::find_if(_camera_list.begin(), _camera_list.end(),
                     [id](const auto &camera) { return camera->id == id; });
  ...
  void set_active_camera(int index) { _active_camera = index; }
  ...
  size_t get_max_id() { return _max_id; }

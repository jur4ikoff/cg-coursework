...
public:
  ...
  RenderSettings(int samples, int depth, Color back)
      : samples_per_pixel(samples), max_depth(depth), background(back) {}
...
private:
  ...
  void initialize() {
    ...
    auto h = std::tan(
        theta / 2); // Высота вычисляется через тангенс половины угла обзора:
    auto viewport_height = 2 * h * _camera.focus_dist;
    auto viewport_width =
        viewport_height *
        (double(image_width) /
         image_height); // Умножем высоту на соотношение сторон

    // Постоение ортогонального базиса камеры
    w = unit_vector(_camera.lookfrom - _camera.lookat);
    u = unit_vector(cross(_camera.vup,
                          w)); // горизонтальный вектор перпендекулярный w и vup
    v = cross(w, u);           // вектор перепендекулярный w и u

    // Векторы, описывающие полный размер viewport в мировых координатах
    Vec3 viewport_u =
        viewport_width * u; // Горизонтальный вектор — ширина viewport
    Vec3 viewport_v =
        viewport_height * -v; // Вертикальный вектор — высота viewport (вниз)
    ...
    // center - focus_dist * w точка на линии взгляда на расстоянии focus_dist
    // от камеры. Это центр плоскости проекции.
    auto viewport_upper_left =
        center - (_camera.focus_dist * w) - viewport_u / 2 - viewport_v / 2;
    pixel00_loc = viewport_upper_left +
                  0.5 * (pixel_delta_u +
                         pixel_delta_v); // Позиция центра первого (0,0) пикселя

    // Вычислени базисных векторов круга расфокусировки
    // _camera.defocus_angle — угол расфокусировки. Делим его на 2, потому что
    // угол измеряется от центральной оси до края круга. Преобразуем в радианы
    // Умножаем на _camera.focus_dist — расстояние до плоскости фокусировк
    auto defocus_radius =
        _camera.focus_dist *
        std::tan(degrees_to_radians(_camera.defocus_angle /
                                    2)); // Радиус круга расфокусировки
  ...
  Ray get_ray(int i, int j) const {
    ...
    auto ray_origin =
        (_camera.defocus_angle <= 0) ? center : defocus_disk_sample();
  ...
  /**
   * @brief Возвращает случайную точку внутри круга радиуса radius, лежащего в
   * плоскости XY и центрированного в начале координат (0,0,0)
   */
  ...
  Color ray_color(const Ray &r, int depth, const Hittable &world,
                  volatile bool &cancel_running) const {
    if (cancel_running)
      return background;
    ...
    Color color_from_scatter =
        attenuation * ray_color(scattered, depth - 1, world, cancel_running);

...
HittableList Scene::make_default_scene() {
  ...
  // _objects.add(make_shared<Smoke>(test, 2, 0.003, *fog_color));
  _objects.add(make_shared<GroundSmoke>(test, 5, 0.003, 0.0032, *fog_color));
  //
  ...
  // auto pyr = irregular_pyramid(Point3(0,0,0), Point3(0, 0, 200), Point3(200,
  // 0, 200), Point3(200, 0, 0), Point3(200, 400, 100), metal_mat);
  // _objects.add(pyr);

  // auto pyr = triangle_pyramid(Point3(0, 0, 0), Point3(0, 0, 200), Point3(200,
  // 0, 200), Point3(100, 200, 100), metal_mat); _objects.add(pyr);

  // auto cylinder_1 = make_shared<Cylinder>(50, 0, 100, cylinder_mat);

  // auto tr = make_shared<triangle>(Point3(0, 0, 0), Point3(100, 400, 100),
  // Point3(200, 0, 200), white); auto tr_r = make_shared<RotateY>(tr, 90);
  // _objects.add(make_shared<Shift>(tr_r, vec3(0, 0, 400)));

  // Cornell box sides
  _objects.add(make_shared<Quad>(Point3(555, 0, 0), Vec3(0, 0, 555),
                                 Vec3(0, 555, 0), light_blue));
  _objects.add(make_shared<Quad>(Point3(0, 0, 555), Vec3(0, 0, -555),
                                 Vec3(0, 555, 0), red));
  _objects.add(make_shared<Quad>(Point3(0, 555, 0), Vec3(555, 0, 0),
                                 Vec3(0, 0, 555), white));
  _objects.add(make_shared<Quad>(Point3(0, 0, 555), Vec3(555, 0, 0),
                                 Vec3(0, 0, -555), white));
  _objects.add(make_shared<Quad>(Point3(555, 0, 555), Vec3(-555, 0, 0),
                                 Vec3(0, 555, 0), white));

  // _objects.add(make_shared<Sphere>(Point3(200, 20, 200)  white));

  // Light
  _objects.add(make_shared<Quad>(Point3(200, 554, 200), Vec3(200, 0, 0),
                                 Vec3(0, 0, 200), light));
  _objects.add(make_shared<Quad>(Point3(200, 1, 200), Vec3(200, 0, 0),
                                 Vec3(0, 0, 200), light));
  ...
  // auto test_box = make_shared<Quad>(Point3(0, 0, 400), vec3(0, 0, -400),
  // vec3(0, 555, 0), red); auto test_box_r = make_shared<RotateY>(test_box,
  // 90); _objects.add(make_shared<Shift>(test_box_r, vec3(0, 0, 400)));
  // shared_ptr<hittable> box1 = box(Point3(0, 0, 0), Point3(100, 400, 100),
  // white); box1 = make_shared<Shift>(box1, vec3(140, 0, 140));
  // _objects.add(box1);
  ...
  // _objects.add(make_shared<ConstantFog>(box1, 0.001, Color(0.25, 0.25,
  // 0.25)));
...
void Scene::add_cylinder(const Point3 &center, double radius, double ymin,
                         double ymax) {
  _objects.add(
      make_shared<Cylinder>(center, radius, ymin, ymax, _default_material));
}
...
void Scene::add_quad_pyramid(const Point3 &center, double half_width,
                             double height) {
  _objects.add(pyramid(center, half_width, height, _default_material));
}

void Scene::add_triag_pyramid(const Point3 &v0, const Point3 &v1,
                              const Point3 &v2, const Point3 &apex) {
...
void Scene::delete_object(size_t id) { _objects.delete_object(id); }

HittableList &Scene::get_objects() { return _objects; }
...
void Scene::rotate_object(size_t id, Vec3 &rotate_info) {
  ...
  std::shared_ptr<Hittable> new_object =
      std::make_shared<RotateX>(object, rotate_info.x());
...
void Scene::add_smoke(size_t obj_id, double density, double scale,
                      Color &fog_color) {
  auto object = _objects.get_object_by_id(obj_id);
...
void Scene::add_ground_smoke(size_t obj_id, double density, double scale,
                             double height_falloff, Color &fog_color) {
  auto object = _objects.get_object_by_id(obj_id);
  _objects.add(make_shared<GroundSmoke>(object, density, scale, height_falloff,
                                        fog_color));
  _objects.delete_object(obj_id);
}

...
public:
  ...
  bool hit(const Ray &r, Interval ray_t, HitRecord &rec) const override {
    // Преобразуем луч из мировых координат в локальные (обратный поворот вокруг
    // Z)
    auto origin =
        Point3((cos_theta * r.origin().x()) + (sin_theta * r.origin().y()),
               (-sin_theta * r.origin().x()) + (cos_theta * r.origin().y()),
               r.origin().z());

    auto direction =
        Vec3((cos_theta * r.direction().x()) + (sin_theta * r.direction().y()),
    ...
    rec.p =
        Point3((cos_theta * rec.p.x()) - (sin_theta * rec.p.y()),
               (sin_theta * rec.p.x()) + (cos_theta * rec.p.y()), rec.p.z());

    rec.normal =
        Vec3((cos_theta * rec.normal.x()) - (sin_theta * rec.normal.y()),
...
public:
  ...
  bool hit(const Ray &r, Interval ray_t, HitRecord &rec) const override {
    ...
    // Преобразуем точку и нормаль обратно в мировые координаты (прямой поворот)
    rec.p = Point3(rec.p.x(), (cos_theta * rec.p.y()) - (sin_theta * rec.p.z()),

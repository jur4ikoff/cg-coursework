...
public:
  ...
  Cone(const Point3 &center, double radius, double height,
       shared_ptr<Material> _mat)
      : cx(center.x()), cz(center.z()), y_min(center.y()),
        y_max(center.y() + height), radius(radius) //, mat(mat)
  {
    // AABB: охватывает весь конус
    auto r = radius;
    bbox = Aaab(Point3(cx - r, y_min, cz - r), Point3(cx + r, y_max, cz + r));

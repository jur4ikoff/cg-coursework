...
public:
  ...
  Cylinder(const Point3 &center, double radius, double y0, double y1,
           shared_ptr<Material> _mat)
      : center(center.x(), 0, center.z()), // x и z — центр оси, y игнорируется
        radius(radius), y_min(std::fmin(y0, y1)), y_max(std::fmax(y0, y1))
  // mat(mat)
  {
  ...
  bool hit(const Ray &r, Interval ray_t, HitRecord &rec) const override {
    ...
    // Пытаемся найти пересечение с боковой поверхностью в пределах [y_min,
    // y_max]
    bool hit_side = false;
    double t = 0;
    Point3 p;
    ...
    if (hit_side) {
      ...
      rec.u =
          std::atan2(outward_normal.z(), outward_normal.x()) / (2 * M_PI) + 0.5;

...
MaterialDialog::MaterialDialog(QWidget *parent)
  ...
  connect(ui->typeComboBox, &QComboBox::currentIndexChanged, this,
          &MaterialDialog::on_typeComboBox_currentIndexChanged);
  connect(ui->lambertianColorButton, &QPushButton::clicked, this,
          &MaterialDialog::on_lambertianColorButton_clicked);
  connect(ui->metalColorButton, &QPushButton::clicked, this,
          &MaterialDialog::on_metalColorButton_clicked);

  updateFields();
}

MaterialDialog::~MaterialDialog() { delete ui; }
...
void MaterialDialog::on_lambertianColorButton_clicked() {
  QColor c =
      QColorDialog::getColor(_lambertianColor, this, "Цвет матового материала");
  if (c.isValid()) {
    _lambertianColor = c;
    ui->lambertianColorPreview->setStyleSheet(
...
void MaterialDialog::on_metalColorButton_clicked() {
  ...
  if (c.isValid()) {
    _metalColor = c;
    ui->metalColorPreview->setStyleSheet(
        QString("background-color: %1;").arg(c.name()));
  }
}
...
double MaterialDialog::refractionIndex() const {
  return ui->iorSpinBox->value();
}
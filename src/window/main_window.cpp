...
void MainWindow::on_changeWorldColor_clicked() {
  QColor selectedColor =
      QColorDialog::getColor(Qt::white, this, "Выберите цвет");
...
void MainWindow::on_CameraAddDialogButton_clicked() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    try {
      auto camera = std::make_shared<Camera>(dialog.vfov(), dialog.lookfrom(),
                                             dialog.lookat(), vup);
...
void MainWindow::on_cameraEditButton_clicked() {
  ...
  CameraAddDialog dialog(camera->lookfrom, camera->lookat, camera->vfov,
                         camera->defocus_angle, camera->focus_dist, this);
  Point3 vup = Point3{0, 1, 0};
  ...
  if (dialog.exec() == QDialog::Accepted) {
    try {
      ...
      QMessageBox::critical(this, "Ошибка",
                            "Невозможно отредактировать камеру.");
    }
    _livetime_render();
  }
}
...
void MainWindow::on_objectChangeMaterialButton_clicked() {
  ...
  case Lambertian_t:
  case Metal_t: {
    QColor qcolor = (mat.type == Lambertian_t) ? dialog.lambertianColor()
                                               : dialog.metalColor();
...
void MainWindow::on_FogAddDialogButton_clicked() {
  ...
  if (ids.size() <= 0) {
    QMessageBox::warning(
        this, "Ошибка",
        "Нет объектов для использования в качестве границы тумана.");
    return;
  }
  ...
  Color fogColor(
      qc.redF(), qc.greenF(),
      qc.blueF()); // предполагается, что Color принимает float в [0,1]
  ...
  try {
    ...
    case FogDialog::GroundHugging:
      _scene->add_ground_smoke(boundary_id, density, dialog.scale(),
                               dialog.heightFalloff(), fogColor);
      break;
    }
    update_objects_list();
    _livetime_render();
...
void MainWindow::_livetime_render() {
  ...
  StartSimpleRenderCommand command{live_render_settings, camera, _world,
                                   *_color_matrix, cancel_live_view_running};
...
void MainWindow::show_error(const std::string label,
                            const std::string message) {
  QMessageBox::critical(this, QString::fromStdString(label),
...
void MainWindow::on_objectAddButton_clicked() {
  ...
  try {
    ...
    case AddObjectDialog::Cylinder:
      qDebug() << "add cylender";
      _scene->add_cylinder(dialog.cylinderAxisCenter(), dialog.cylinderRadius(),
                           dialog.cylinderY0(), dialog.cylinderY1());
      break;

    case AddObjectDialog::Cone:
      qDebug() << "add cone";
      _scene->add_cone(dialog.coneBaseCenter(), dialog.coneRadius(),
                       dialog.coneHeight());
      break;

    case AddObjectDialog::QuadPyramid:
      qDebug() << "add quad pyramid";
      _scene->add_quad_pyramid(dialog.quadPyramidBaseCenter(),
    ...
    case AddObjectDialog::TriPyramid:
      ...
      _scene->add_triag_pyramid(dialog.triPyramidV0(), dialog.triPyramidV1(),
                                dialog.triPyramidV2(), dialog.triPyramidApex());
      break;
    ...
    case AddObjectDialog::Quad:
      qDebug() << "add quad";
      _scene->add_quad(dialog.quadQ(), dialog.quadU(), dialog.quadV());
      break;

    case AddObjectDialog::Triangle:
      qDebug() << "add triag";
      _scene->add_triangle(dialog.triA(), dialog.triB(), dialog.triC());
      break;
    }

...
Point3 AddObjectDialog::sphereCenter() const {
  return Point3(ui->sphereCenterX->value(), ui->sphereCenterY->value(),
                ui->sphereCenterZ->value());
}
...
double AddObjectDialog::cylinderRadius() const {
  return ui->cylinderRadius->value();
}

// Конус
Point3 AddObjectDialog::coneBaseCenter() const {
  return Point3(ui->coneBaseX->value(), ui->coneBaseY->value(),
                ui->coneBaseZ->value());
}
...
Point3 AddObjectDialog::quadPyramidBaseCenter() const {
  return Point3(ui->quadPyramidCenterX->value(),
                ui->quadPyramidCenterY->value(),
                ui->quadPyramidCenterZ->value());
}
double AddObjectDialog::quadPyramidHalfWidth() const {
  return ui->quadPyramidHalfWidth->value();
}
double AddObjectDialog::quadPyramidHeight() const {
  return ui->quadPyramidHeight->value();
}
...
Point3 AddObjectDialog::triPyramidApex() const {
  return Point3(ui->triApexX->value(), ui->triApexY->value(),
                ui->triApexZ->value());
}
...
// Для Quad: u = P1 - Q, v = P2 - Q, но в UI вы вводите компоненты U и V
// напрямую Однако ваш UI вводит именно векторы U и V (подписано как "U X", "V
// X" и т.д.) Поэтому:
...
AddObjectDialog::ObjectType AddObjectDialog::selectedType() const {
  ...
  static const ObjectType mapping[] = {Sphere,     Cylinder, Cone, QuadPyramid,
                                       TriPyramid, Box,      Quad, Triangle};
  int idx = ui->typeComboBox->currentIndex();
  if (idx >= 0 &&
      idx < static_cast<int>(sizeof(mapping) / sizeof(mapping[0]))) {
    return mapping[idx];
  }
  return Sphere;
}
...
public:
    ...
    ColorMatrix(size_type rows_count, size_type columns_count, const Color &default_color)
    {
        _rows_count = rows_count;
        _columns_count = columns_count;
    ...
    // Изменение размера
    void resize(size_type new_rows, size_type new_columns, const Color &default_color = Color())
    {
    ...
    std::vector<Color> &operator[](size_type row)
    {
        return _data[row];
    }
